    ...
    try:
        while i <= 39:
            try:
                ...
                if Size[0] != '1' and price_per_sqft != 'Please Contact':
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Create a table to store rental data
        create_table_query = '''CREATE TABLE IF NOT EXISTS rental_data_analysis
                     (Title VARCHAR(255), Website VARCHAR(255), Price TEXT, Price_per_square_foot TEXT, Type TEXT, Post_Time TEXT, Location TEXT, Latitude TEXT, Longitude TEXT,
                      Bedroom TEXT, Bathroom TEXT, Utilities_Included TEXT, Wi_Fi_and_More TEXT, Parking_Included TEXT, Agreement_Type TEXT,
                      Move_In_Date TEXT, Pet_Friendly TEXT, Size_sqft TEXT, Furnished TEXT, Appliances TEXT, Air_Conditioning TEXT,
                      Personal_Outdoor_Space TEXT, Smoking_Permitted TEXT,
                      UNIQUE KEY (Title(255), Website(255)))'''

        cursor.execute(create_table_query)
        conn.commit()

        # Insert data into the database with handling duplicates
        insert_query = '''INSERT INTO rental_data_analysis (Title, Website, Price, Price_per_square_foot, Type, Post_Time, Location, Latitude, Longitude, 
                                    Bedroom, Bathroom, Utilities_Included, Wi_Fi_and_More, Parking_Included, Agreement_Type, Move_In_Date, Pet_Friendly, 
                                    Size_sqft, Furnished, Appliances, Air_Conditioning, Personal_Outdoor_Space, Smoking_Permitted) 
                                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                                    ON DUPLICATE KEY UPDATE
                                    Title = VALUES(Title), Price = VALUES(Price), Price_per_square_foot = VALUES(Price_per_square_foot), Type = VALUES(Type),
                                    Post_Time = VALUES(Post_Time), Location = VALUES(Location), Latitude = VALUES(Latitude), Longitude = VALUES(Longitude),
                                    Bedroom = VALUES(Bedroom), Bathroom = VALUES(Bathroom), Utilities_Included = VALUES(Utilities_Included),
                                    Wi_Fi_and_More = VALUES(Wi_Fi_and_More), Parking_Included = VALUES(Parking_Included), Agreement_Type = VALUES(Agreement_Type),
                                    Move_In_Date = VALUES(Move_In_Date), Pet_Friendly = VALUES(Pet_Friendly), Size_sqft = VALUES(Size_sqft), Furnished = VALUES(Furnished),
                                    Appliances = VALUES(Appliances), Air_Conditioning = VALUES(Air_Conditioning), Personal_Outdoor_Space = VALUES(Personal_Outdoor_Space),
                                    Smoking_Permitted = VALUES(Smoking_Permitted)'''

        for item in flattened_results:
            cursor.execute(insert_query, (
                item['Title'], item['Website'], item['Price'], item['Price per square foot'], item['Type'],
                item['Post Time'],
                item['Location'], item['Latitude'], item['Longitude'], item['Bedroom'], item['Bathroom'],
                item['Utilities Included'],
                item['Wi Fi and More'], item['Parking Included'], item['Agreement Type'], item['Move In Date'],
                item['Pet Friendly'],
                item['Size(sqft)'], item['Furnished'], item['Appliances'], item['Air Conditioning'],
                item['Personal Outdoor Space'],
                item['Smoking Permitted']))
